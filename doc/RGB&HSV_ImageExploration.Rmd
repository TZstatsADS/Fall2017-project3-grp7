...
setwd("~/Desktop/Fall2107-Proj3-Grp7/data/training_set/images")
...
# Test on these 9 images by usnig GBM model
if(!require(gbm)){
  install.packages('gbm')
  library('gbm')
}

labels<-c(0,0,0,1,1,1,2,2,2)    # '0' for chicken, '1' for dog, '2' for muffin
gbm_model<- gbm(labels~., 
                data=data.frame(RGB_features),
                interaction.depth=3,
                shrinkage=0.02,
                n.trees=600,
...
pred<- predict(gbm_model, 
               data.frame(RGB_features),
               n.trees=gbm_model$n.trees,
               type='response')
sum(apply(pred, 1, which.max) - 1 == labels) / length(labels)
```
